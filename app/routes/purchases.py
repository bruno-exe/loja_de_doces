...
def seller_purchases_page(request: Request, seller_id: int):
    ...
    with SessionLocal() as database:
        ...
        orders = [{"id": order.id, "nome": order.produto_nome, "imagem": order.produto_imagem or (product.imagem if product else None), "quantidade": order.quantidade, "situacao": "Pago" if order.pago else "Pagamento pendente", "pago": order.pago} for order, product in rows]
